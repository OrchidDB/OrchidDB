...
pub(super) fn gvalue_to_lit(value: &GValue) -> GremlinPlanResult<Lit> {
    Ok(match value {
        GValue::SackCallbacks { .. } | GValue::CardinalityValue {..} | GValue::Token(_) | GValue::DirectionToken(_) | GValue::TypedMap(_) | GValue::VertexPropertyRef { .. } | GValue::VertexRef { .. } | GValue::EdgeRef { .. } => return Err(crate::language::gremlin::planner::error::GremlinPlanError::Unsupported("typed Gremlin value requires runtime evaluation".into())),
...
pub(super) fn gvalue_to_expr(value: &GValue) -> GremlinPlanResult<IrExpr> {
    Ok(match value {
        ...
        GValue::VertexPropertyRef { id, owner, key } => IrExpr::Call { name: "gremlin_vertex_property_ref".into(), args: vec![gvalue_to_expr(id)?, gvalue_to_expr(owner)?, IrExpr::lit_str(key)] },
        GValue::EdgeRef { id } => IrExpr::Call { name: "gremlin_edge_ref".into(), args: vec![gvalue_to_expr(id)?] },
...
pub(super) fn gvalue_to_value(value: &GValue) -> Option<Value> {
    Some(match value {
        ...
        GValue::SackCallbacks { .. } | GValue::VertexPropertyRef { .. } | GValue::VertexRef { .. } | GValue::EdgeRef { .. } => return None,
