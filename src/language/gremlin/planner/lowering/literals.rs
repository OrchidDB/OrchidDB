...
pub(super) fn gvalue_to_lit(value: &GValue) -> GremlinPlanResult<Lit> {
    Ok(match value {
        GValue::CardinalityValue {..} | GValue::Token(_) | GValue::DirectionToken(_) | GValue::TypedMap(_) | GValue::VertexRef { .. } | GValue::EdgeRef { .. } => return Err(crate::language::gremlin::planner::error::GremlinPlanError::Unsupported("typed Gremlin value requires runtime evaluation".into())),
...
pub(super) fn gvalue_to_value(value: &GValue) -> Option<Value> {
    Some(match value {
        ...
        GValue::VertexRef { .. } | GValue::EdgeRef { .. } => return None,
