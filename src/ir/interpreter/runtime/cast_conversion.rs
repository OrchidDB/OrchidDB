...
pub(super) fn cast_conversion_error() -> InterpretError {
    InterpretError::Runtime("Conversion exception:".to_string())
}

pub(super) fn cast_overflow_error() -> InterpretError {
    InterpretError::Runtime("Overflow exception:".to_string())
}

pub(super) fn cast_conversion_to_type_error(value: &Value, target_type: &str) -> InterpretError {
...
pub(super) fn cast_range_error(value: i128, target_type: &str) -> InterpretError {
    InterpretError::Runtime(format!(
        "Overflow exception: Value {value} is not within {target_type} range"
    ))
}

pub(super) fn cast_bigint_range_error(value: &num_bigint::BigInt, target_type: &str) -> InterpretError {
    InterpretError::Runtime(format!(
        "Overflow exception: Value {value} is not within {target_type} range"
    ))
}

pub(super) fn cast_negative_unsigned_error(value: &num_bigint::BigInt) -> InterpretError {
    InterpretError::Runtime(format!(
        "Overflow exception: Cast failed. Cannot cast {value} to unsigned type."
    ))
}

pub(super) fn cast_negative_int128_to_uint128_error(value: &num_bigint::BigInt) -> InterpretError {
    InterpretError::Runtime(format!(
