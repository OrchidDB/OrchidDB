...
pub(super) fn cypher_call(
    ...
    match (canonical.as_ref(), args) {
        ("cypher_slice_bound", [value]) => {
            ...
            let integer=super::lists::range_integer_arg(value).ok_or_else(||RuntimeError::Diagnosed {
                code:RuntimeDiagnosis::InvalidSliceArgument,message:"SKIP/LIMIT require an integer".into(),
            })?;
            if integer<0 {return Err(RuntimeError::Diagnosed {code:RuntimeDiagnosis::NegativeIntegerArgument,
        ...
        ("cypher_percentile_fraction", [value]) => {
            use crate::ir::diagnostics::RuntimeDiagnosis;
            let fraction = value_as_f64(value).ok_or_else(|| RuntimeError::Diagnosed {
                code: RuntimeDiagnosis::ArgumentType,
                message: "Percentile fraction must be numeric".into(),
            })?;
            if !fraction.is_finite() || !(0.0..=1.0).contains(&fraction) {
                return Err(RuntimeError::Diagnosed {
        ...
        ("parameter", [Value::String(name)]) => Err(RuntimeError::Runtime(format!(
        ...
        ("cypher_in", [_, _]) => Err(RuntimeError::Diagnosed {
        ...
        ("list_transform", [_list, _not_lambda]) => Err(RuntimeError::Runtime(
            "Binder exception: The second argument of LIST_TRANSFORM should be a lambda expression but got LITERAL."
                .to_string(),
        )),
        ("error", [Value::String(message)]) => {
            Err(RuntimeError::Runtime(format!("Runtime exception: {message}")))
        }
        ("error", [message]) => Err(RuntimeError::Runtime(format!(
            "Runtime exception: {}",
        ...
        ("add5", args) if args.len() != 1 => Err(RuntimeError::Runtime(
            "Binder exception: Invalid number of arguments for macro ADD5.".to_string(),
        )),
        ("add4", args) if args.len() > 3 => Err(RuntimeError::Runtime(
        ...
        ("decode", [v]) => {
            ...
            match String::from_utf8(bytes) {
                ...
                Err(_) => Err(RuntimeError::Runtime(
        ...
        ("concat_ws", args) if args.len() < 2 => Err(RuntimeError::Runtime(format!(
...
fn cypher_temporal_field(
    ...
    match fields.get(key) {
        Some(Value::Float(_) | Value::Float32(_) | Value::BigDecimal(_)) => {
            Err(RuntimeError::Type(format!("{key} must be an integer")))
        }
        Some(value) => value
            .as_i64()
            .ok_or_else(|| RuntimeError::Type(format!("{key} must be an integer"))),
        None => {
            default.ok_or_else(|| RuntimeError::Runtime(format!("missing temporal field {key}")))
        }
    }
}
...
fn cypher_calendar_date(fields: &std::collections::BTreeMap<String, Value>) -> IrResult<String> {
    ...
    temporal::make_date(year, month, day)
        .ok_or_else(|| RuntimeError::Runtime("invalid calendar date".into()))
}
...
fn cypher_calendar_datetime(
    ...
    {
        return Err(RuntimeError::Runtime("invalid calendar time".into()));
    }
    let timezone = match fields.get("timezone") {
        None => "Z",
        Some(Value::String(value)) => value.as_str(),
        Some(_) => return Err(RuntimeError::Type("timezone must be a string".into())),
    };
    ...
    parse_datetime_string(&value)
        .ok_or_else(|| RuntimeError::Unsupported(format!("datetime timezone {timezone}")))
}
    ...
    let seconds = match fields.get("seconds") {
        Some(Value::Float(value)) if value.is_finite() => {
            ...
            if scaled.abs() > i64::MAX as f64 * 1_000_000_000.0 {
                return Err(RuntimeError::Runtime("duration seconds overflow".into()));
            }
...
fn cypher_duration_fields(source: &str) -> IrResult<std::collections::BTreeMap<String, Value>> {
    let invalid = || RuntimeError::Runtime(format!("invalid ISO duration {source}"));
