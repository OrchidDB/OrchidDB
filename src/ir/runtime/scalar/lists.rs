...
pub(super) fn cypher_subscript(
    ...
    if let Value::List(items) = target {
        return match range_integer_arg(index) {
            ...
            None => Err(RuntimeError::Diagnosed {
...
pub(super) fn list_extract_type_error() -> RuntimeError {
...
pub(super) fn gremlin_merge(lhs: &Value, rhs: &Value, traversal: bool) -> IrResult<Value> {
    ...
    if is_map(lhs) {
        if !is_map(rhs) {
            return Err(RuntimeError::Runtime(format!("merge step expected provided argument to evaluate to a Map, encountered {}", rhs.type_name())));
        }
    ...
    let lhs = iterable(lhs).ok_or_else(|| RuntimeError::Runtime(if matches!(lhs,Value::Null) {
    ...
    if !traversal && is_map(rhs) {
        return Err(RuntimeError::Runtime("merge step type mismatch: expected argument to be Iterable but got Map".into()));
    }
    let rhs = iterable(rhs).ok_or_else(|| RuntimeError::Runtime(if traversal {
