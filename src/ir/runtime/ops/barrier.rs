...
pub(crate) fn barrier_op(
    ...
    let _ = materialize; // this native kernel receives already materialized rows.
        ...
        match sigs.get(&sig).copied() {
            Some(prev) => {
                ...
                rows[prev].bulk = rows[prev].bulk.checked_add(bulk).ok_or_else(|| {
                    super::super::RuntimeError::ExecutionLimit("traverser bulk overflow".into())
                })?;
                keep[idx] = false;
            }
            None => {
...
fn gremlin_barrier(rows: Vec<Row>, normalize: bool, graph: &PropertyGraph) -> IrResult<Vec<Row>> {
    use super::super::scalar::eval_call;
...
mod bulk_tests {
    ...
    fn compaction_rejects_bulk_overflow() {
        ...
        assert!(matches!(
            ...
            Err(super::super::super::RuntimeError::ExecutionLimit(_))
        ));
    }
}
