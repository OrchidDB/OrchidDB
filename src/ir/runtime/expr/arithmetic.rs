...
use super::super::scalar::{runtime_list, temporal};
use super::super::{RuntimeError, IrResult};
...
pub(crate) fn arithmetic(op: BinaryOp, lhs: &Value, rhs: &Value) -> IrResult<Value> {
    ...
    if matches!(lhs, Value::Temporal(_)) || matches!(rhs, Value::Temporal(_)) {
        return crate::ir::temporal::arithmetic(op, lhs, rhs).map_err(RuntimeError::Type);
    }
...
fn modulo_by_zero_error() -> RuntimeError {
    RuntimeError::Runtime("Runtime exception: Modulo by zero.".into())
}
...
fn integer_overflow_error(
    ...
    if matches!(kind, IntegerKind::I128 | IntegerKind::U128) {
        return RuntimeError::Runtime(format!(
...
fn checked_int(value: Option<i64>, operation: &str) -> IrResult<Value> {
    value
        .map(Value::Int)
        .ok_or_else(|| RuntimeError::Type(format!("integer overflow during {operation}")))
}
...
mod tests {
    ...
    fn int_overflow_is_reported_not_panicked() {
        let err = arithmetic(BinaryOp::Mul, &Value::Int(i64::MAX), &Value::Int(2))
            .expect_err("overflow should be classified as a runtime error");
