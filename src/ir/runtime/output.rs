...
use super::scalar::display_for_concat;
use super::{RuntimeError, IrResult, ReturnedBatches, Row};
    ...
    if gremlin_elements || expand_elements {
        schema.metadata.insert(
            ...
            serde_json::to_string(fields)
                .map_err(|err| RuntimeError::Type(format!("typed result columns: {err}")))?,
        );
        ...
        schema.metadata.insert(
            ...
            serde_json::to_string(&typed_rows)
                .map_err(|err| RuntimeError::Type(format!("typed Gremlin results: {err}")))?,
        );
    }
    let schema: SchemaRef = Arc::new(schema);
    let batch = RecordBatch::try_new(schema, arrays)
        .map_err(|err| RuntimeError::Type(format!("record batch: {err}")))?;
    Ok(ReturnedBatches {
