...
#[test]
fn choice_and_productive_where_use_gremlin_numeric_promotion() {
    let graph = PropertyGraph::new();
    assert_rows(
        &graph,
        "g.inject(0.1M).choose(__.identity()).option(0.1D,__.constant('match'))",
        &["match"],
    );
    assert_rows(
        &graph,
        "g.withStrategies(ProductiveByStrategy).inject(0.1M).as('a').constant(0.1D).as('b').where('a',P.eq('b')).constant('match')",
        &["match"],
    );
}
