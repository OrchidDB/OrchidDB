//! Gremlin TinkerPop case runner — wired into `cargo test`.
//!
//! Walks every `*.case` file under `cases/gremlin/tinkerpop`, parses the
//! embedded Gremlin query, plans it through `GremlinPlanner`, executes
//! it against an in-memory `PropertyGraph` built from the case's dataset
...
//! common error messages so we can see which Gremlin features dominate
//! the gap.
//!
//! See output with:
//!
//! ```ignore
//! cargo test --test gremlin_tinkerpop_cases -- --nocapture
//! ```
//!
...
const CASES_ROOT: &str = "cases/gremlin/tinkerpop";
...
const FAILURES_DIR: &str = "target/gremlin_case_failures";

#[test]
fn gremlin_tinkerpop_cases() {
    ...
    if !root.exists() {
        ...
        eprintln!(
            "[gremlin_tinkerpop_cases] skipping: cases directory `{}` not present",
            root.display()
        );
        return;
    }
    ...
    walk_cases(&root, &mut |path| {
        ...
        if !matches!(run.outcome, Outcome::Correct) {
            if let Err(err) = dump_failure(&failures_dir, path, &run) {
                eprintln!(
                    "[gremlin_tinkerpop_cases] failed to write failure file for `{}`: {err}",
                    path.display()
                );
            }
        }
...
/// useful: the original Gremlin query string and (when planning got that
...
fn run_one(path: &Path) -> CaseRun {
    ...
    if case.metadata.language != "gremlin" {
        return CaseRun {
            ...
            plan_tree: None,
            outcome: Outcome::Skipped(format!("non-gremlin language: {}", case.metadata.language)),
        };
    }

    let query = case.query.clone();
    let traversal = match parse::gremlin_with_case(&case.query, &case.metadata.source_case) {
        Ok(t) => t,
        Err(err) => {
            return CaseRun {
                query: Some(query),
...
struct Summary {
    ...
    /// Per-suite breakdown (e.g., branch/Branch, filter/Not, …).
...
impl Summary {
    ...
    fn assert_no_incorrect(&self) {
        if self.incorrect > 0 {
            panic!(
                "{} TinkerPop case(s) produced incorrect data — see breakdown above",
                self.incorrect
            );
        }
    }
...
fn suite_label(path: &Path) -> String {
    // cases/gremlin/tinkerpop/<group>/<feature>/file.case
...
/// original Gremlin query, the IR plan tree (when planning got that far),
...
/// `cases/gremlin/tinkerpop/` prefix when present and replaces path
