...
async fn typed_traversers_and_correlated_state_match_reference() {
    for query in [
        ...
        let plan = plan(query);
        let graph = graph();
        let mut expected = orchiddb::ir::interpreter::execute_rows(&plan, &graph.clone()).unwrap();
...
async fn fused_unary_kernels_preserve_exact_rows_and_bulk() {
    ...
    let graph = PropertyGraph::new();
    let expected = orchiddb::ir::interpreter::execute_rows(&query, &graph).unwrap();
...
async fn ranking_windows_allow_unordered_and_ordered_dedup() {
    for query in [
        ...
        let graph = graph();
        let plan = plan(query);
        let expected = orchiddb::ir::interpreter::execute_rows(&plan, &graph.clone()).unwrap();
        ...
        let visible = |rows: Vec<orchiddb::ir::interpreter::Row>| {
...
async fn batched_apply_preserves_occurrences_bulk_and_optional_results() {
    use orchiddb::ir::expr::{IrExpr, Lit, BinaryOp};
    use orchiddb::ir::plan::{Node, ApplyKind, ProjectMode, ProjectErrorPolicy, ProjectionItem};
    ...
    for kind in [ApplyKind::Inner,ApplyKind::Optional,ApplyKind::Semi,ApplyKind::Anti,ApplyKind::Scalar] {
        ...
        let graph=PropertyGraph::new();
        let expected=orchiddb::ir::interpreter::execute_rows(&plan,&graph).unwrap();
...
async fn projection_composition_preserves_shadowing_and_missing_bindings() {
    ...
    use orchiddb::ir::plan::{Node, ProjectMode, ProjectErrorPolicy, ProjectionItem};
...
async fn required_bindings_preserve_projection_inputs_and_errors() {
    use orchiddb::ir::expr::{IrExpr, Lit, BinaryOp};
    use orchiddb::ir::plan::{Node, ProjectMode, ProjectErrorPolicy, ProjectionItem};
    use orchiddb::ir::policy::{GraphPlanPolicy,ResultForm,PropertyMissing};
    ...
    let expected=orchiddb::ir::interpreter::execute(&plan,&graph).unwrap();
    let (actual,stats)=orchiddb::ir::rel::runtime::execute(&plan,&graph,None).await.unwrap();
    ...
    assert!(orchiddb::ir::interpreter::execute(&error_plan,&graph).is_err());
    let error=orchiddb::ir::rel::runtime::execute(&error_plan,&graph,None).await.unwrap_err();
...
async fn batched_fanout_keeps_duplicate_outputs_and_parent_bulk() {
    ...
    use orchiddb::ir::plan::{Node,ApplyKind};
    use orchiddb::ir::policy::{GraphPlanPolicy,OptionalMissing};
    ...
    let plan=GraphPlan::new(GraphPlanPolicy::gremlin(),Node::GraphApply {kind:ApplyKind::Inner,correlation:vec!["current".into()],outputs:vec!["answer".into()],optional_missing:OptionalMissing::Null,
        ...
        right:Box::new(Node::GraphUnwind {input_expr:IrExpr::Binding("current".into()),bind:"answer".into(),outer:false,
            input:Box::new(Node::GraphCorrelate {bindings:vec!["current".into()]})})});
    let graph=PropertyGraph::new();let expected=orchiddb::ir::interpreter::execute_rows(&plan,&graph).unwrap();
    let (actual,stats)=execute_rows_with_jvm(&plan,&graph,JvmExecution::default()).await.unwrap();
