...
public class OrchidGraphTest {
    private String executable;
    @Before public void nativeBinary() {
        executable=System.getenv("ORCHIDDB_JVM_STORE");
        Assume.assumeTrue("ORCHIDDB_JVM_STORE is required for native integration",executable!=null);
    }
    @Test public void configuredDefaultCardinalityAppliesToTraversalCreationAndFreshGraphs() {
        ...
        config.setProperty("orchiddb.native.executable",executable);
        config.setProperty(OrchidGraph.DEFAULT_CARDINALITY,"list");
        try(OrchidGraph graph=OrchidGraph.open(config); OrchidGraph fresh=graph.freshGraph()) {
    ...
    @Test public void constructorDuplicatePropertiesAlwaysUseListCardinality() {
        for(String cardinality:Arrays.asList("single","list","set")) {
            org.apache.commons.configuration2.BaseConfiguration config=new org.apache.commons.configuration2.BaseConfiguration();
            config.setProperty("orchiddb.native.executable",executable);
            config.setProperty(OrchidGraph.DEFAULT_CARDINALITY,cardinality);
            try(OrchidGraph graph=OrchidGraph.open(config)) {
    ...
    @Test public void configuredIdManagersCoerceSuppliedIdsAndPreserveTypedDefaults() {
        org.apache.commons.configuration2.BaseConfiguration config=new org.apache.commons.configuration2.BaseConfiguration();
        config.setProperty("orchiddb.native.executable",executable);
        config.setProperty(OrchidGraph.VERTEX_ID_MANAGER,"LONG");
        config.setProperty(OrchidGraph.EDGE_ID_MANAGER,"INTEGER");
        config.setProperty(OrchidGraph.VERTEX_PROPERTY_ID_MANAGER,"INTEGER");
        try(OrchidGraph graph=OrchidGraph.open(config); OrchidGraph fresh=graph.freshGraph()) {
    ...
    @Test public void configuredNumericManagersGenerateVertexAndEdgeIds() {
        for(String vertexManager:Arrays.asList("INTEGER","LONG")) {
            org.apache.commons.configuration2.BaseConfiguration config=new org.apache.commons.configuration2.BaseConfiguration();
            config.setProperty("orchiddb.native.executable",executable);
            config.setProperty(OrchidGraph.VERTEX_ID_MANAGER,vertexManager);
            config.setProperty(OrchidGraph.EDGE_ID_MANAGER,vertexManager.equals("INTEGER")?"LONG":"INTEGER");
            try(OrchidGraph graph=OrchidGraph.open(config)) {
    ...
    @Test public void numericAutoIdsSkipSuppliedCollisionsAcrossRollbackAndPersistentReopen() throws Exception {
        Path directory=Files.createTempDirectory("orchiddb-numeric-ids-");
        org.apache.commons.configuration2.BaseConfiguration config=new org.apache.commons.configuration2.BaseConfiguration();
        config.setProperty("orchiddb.native.executable",executable);
        config.setProperty("orchiddb.native.path",directory.resolve("graph.ngsp").toString());
        config.setProperty(OrchidGraph.VERTEX_ID_MANAGER,"LONG");
        config.setProperty(OrchidGraph.EDGE_ID_MANAGER,"INTEGER");
    ...
    @Test public void cachedAndUncachedTraversalPreserveTypedIdsOrderAndMultiplicity() {
        String key="orchiddb.native.adjacencyCacheSize",previous=System.getProperty(key);
    ...
    @Test public void cachedAdjacencyDoesNotHideNativeProcessDeath() throws Exception {
        OrchidGraph graph=OrchidGraph.open(executable);
        try {
            Vertex a=graph.addVertex(),b=graph.addVertex(); a.addEdge("link",b);
            assertTrue(a.edges(Direction.OUT).hasNext());
            java.lang.reflect.Field sessionField=OrchidGraph.class.getDeclaredField("session"); sessionField.setAccessible(true);
            java.lang.reflect.Field processField=OrchidSession.class.getDeclaredField("process"); processField.setAccessible(true);
    ...
    @Test public void durableCommitAndCloseRollback() throws Exception {
        Path directory=Files.createTempDirectory("orchiddb-provider-"); Path snapshot=directory.resolve("graph.ngsp");
        try {
            try(OrchidGraph graph=OrchidGraph.open(executable,snapshot)) {
    ...
    @Test public void runtimeObjectsHaveDedicatedTypedSessionReferences() {
        try(OrchidGraph graph=OrchidGraph.open(executable)) {
            ...
            try(OrchidGraph child=graph.freshGraph()) { assertSame(runtime,child.decodeValue(encoded)); }
            Vertex v=graph.addVertex(); v.property("runtime",runtime);
            assertSame(runtime,v.property("runtime").value());
            Map<String,String> ordinary=Collections.singletonMap("orchiddb:runtime:fake","123");
    ...
    @Test public void resolvesExplicitCrossGraphElementReferencesByPublicIdentity() {
        try(OrchidGraph source=OrchidGraph.open(executable); OrchidGraph target=source.freshGraph()) {
    ...
    @Test public void resultGraphAndRuntimeOutliveOriginalGraph() {
        OrchidGraph source=OrchidGraph.open(executable); OrchidGraph result=source.freshGraph();
        try {
            org.apache.tinkerpop.gremlin.process.traversal.traverser.util.TraverserSet<Object> runtime=new org.apache.tinkerpop.gremlin.process.traversal.traverser.util.TraverserSet<>();
    ...
    @Test public void queuedExecutionLeaseCanBeCancelledWithoutAbortingGraph() throws Exception {
        try(OrchidGraph graph=OrchidGraph.open(executable); AutoCloseable lease=graph.executionLease()) {
    ...
    @Test public void closeAndAbortInvalidateNativeSession() {
        OrchidGraph graph=OrchidGraph.open(executable); graph.addVertex(); graph.abort();
