...
public class OrchidSessionTest {
    @Test public void interruptedReplyIsDrainedBeforeNextRequest() throws Exception {
        Path directory=Files.createTempDirectory("orchiddb-protocol-");
        ...
        try(OrchidSession session=new OrchidSession(script.toString(),null)) {
            AtomicBoolean cancelled=new AtomicBoolean(),interrupted=new AtomicBoolean();
            AtomicReference<Throwable> failure=new AtomicReference<>();
            Thread worker=new Thread(()->{
                try { session.call(OrchidCodec.fields("op","slow")); }
            ...
            assertEquals("reply",session.call(OrchidCodec.fields("op","next")));
            Thread.currentThread().interrupt();
            try { session.call(OrchidCodec.fields("op","never-sent")); fail(); }
            catch(CancellationException expected) { assertTrue(Thread.interrupted()); }
            finally { Thread.interrupted(); }
            assertEquals("reply",session.call(OrchidCodec.fields("op","still-open")));
