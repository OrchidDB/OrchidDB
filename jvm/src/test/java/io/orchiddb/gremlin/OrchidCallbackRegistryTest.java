...
public class OrchidCallbackRegistryTest {
    @Test public void originalJavaFunctionsUseBorrowedGraphAndPropagateFailure() throws Exception {
        String nativeStore=System.getenv("ORCHIDDB_JVM_STORE");
        Assume.assumeNotNull(nativeStore);
        try(OrchidGraph graph=OrchidGraph.open(nativeStore);OrchidCallbackRegistry registry=new OrchidCallbackRegistry()) {
