...
public class CrabJvmExecutorTest {
    private static final Duration TIMEOUT=Duration.ofSeconds(20);
    private String binary() {
        String path=System.getenv("CRABGRAPH_JVM_STORE");
    ...
    @Test public void callbacksReadMutateAndCaptureValues() throws Exception {
        try (var session=new CrabJvmExecutor(CrabGraph.open(binary()))) {
    ...
    @Test public void cancelledPersistentSubmissionCannotCommitPartialWrites() throws Exception {
        String store=binary(); var directory=Files.createTempDirectory("crabgraph-jvm-cancel");
        var snapshot=directory.resolve("graph.snapshot");
        try {
            try (var session=new CrabJvmExecutor(CrabGraph.open(store,snapshot))) {
            ...
            try (var graph=CrabGraph.open(store,snapshot)) {
