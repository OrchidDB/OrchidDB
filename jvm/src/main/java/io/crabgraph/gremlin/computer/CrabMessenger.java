 ...
 */
public final class CrabMessenger<M> implements Messenger<M> {

    private final Vertex vertex;
    private final CrabMessageBoard<M> messageBoard;
    private final MessageCombiner<M> combiner;

    public CrabMessenger(final Vertex vertex, final CrabMessageBoard<M> messageBoard, final Optional<MessageCombiner<M>> combiner) {
        this.vertex = vertex;
...
//        for (final MessageScope messageScope : this.messageBoard.previousMessageScopes) {
            if (messageScope instanceof MessageScope.Local) {
                ...
                final Traversal.Admin<Vertex, Edge> incidentTraversal = CrabMessenger.setVertexStart(localMessageScope.getIncidentTraversal().get().asAdmin(), this.vertex);
                final Direction direction = CrabMessenger.getDirection(incidentTraversal);
