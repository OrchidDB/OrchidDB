 ...
 */
public final class CrabCallbackRegistry implements AutoCloseable {
    ...
    private final ExecutorService clients=Executors.newCachedThreadPool(r->{Thread t=new Thread(r,"crabgraph-java-callback");t.setDaemon(true);return t;});
    public CrabCallbackRegistry() throws IOException {
        ...
        },"crabgraph-java-callback-listener");listener.setDaemon(true);listener.start();
    }
    public String register(Object callback) {
        ...
        return "{ Object... args -> io.crabgraph.gremlin.CrabCallbackRegistry.invoke("+server.getLocalPort()+",'"+token+"','"+id+"',args,graph) }";
    }
    @SuppressWarnings("unchecked")
    private void serve(Socket socket) {
        CrabGraph graph=null;
        try {
            ...
            graph=CrabGraph.forIr(input,output);
            ...
            else if(callback instanceof BiFunction<?,?,?> function){if(args.size()!=2)throw new IllegalArgumentException("BiFunction arity");result=((BiFunction<Object,Object,Object>)function).apply(CrabCodec.decode(args.get(0),graph),CrabCodec.decode(args.get(1),graph));}
            else {if(args.size()!=1)throw new IllegalArgumentException("Function arity");result=((Function<Object,Object>)callback).apply(CrabCodec.decode(args.get(0),graph));}
            write(output,CrabCodec.fields("kind","result","ok",true,"value",CrabCodec.encode(result,graph)));
    ...
    @SuppressWarnings("unchecked")
    public static Object invoke(int port,String token,String id,Object[] arguments,CrabGraph graph) throws IOException {
        try(Socket socket=new Socket()) {
            socket.connect(new InetSocketAddress("127.0.0.1",port),5000);socket.setSoTimeout(30000);
            List<Object> args=new ArrayList<>();for(Object arg:arguments)args.add(CrabCodec.encode(arg,graph));
            ...
            while(true) {
                ...
                if("native".equals(response.get("kind"))) {
                    ...
                    try {reply=CrabCodec.fields("ok",true,"value",graph.callbackRequest((Map<String,Object>)response.get("request")));}
                    ...
                    return CrabCodec.decode(response.get("value"),graph);
