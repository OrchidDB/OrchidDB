...
final class CrabComputerKernel {
    private CrabComputerKernel() { }
    @SuppressWarnings({"unchecked","rawtypes"})
    static List<Object> execute(Map<String,Object> request, CrabGraph graph, GremlinGroovyScriptEngine engine) throws Exception {
        ...
        for(int i=0;i<rows.size();i++) {
            Object current=CrabCodec.decode(rows.get(i).get("current"),graph);
        ...
        for(Vertex vertex:input) {
            ...
            for(Object value:results.getOrDefault(vertex.id(),new LinkedHashSet<>()))group.add(CrabCodec.encode(value,graph));
            output.add(group);
        }
        return output;
    }
    ...
    private static Object rebind(Object value,CrabGraph graph) {
