package io.orchiddb.gremlin.computer;
...
/**
 * Single-worker bulk-synchronous executor over a OrchidDB provider graph.
...
public final class OrchidGraphComputer implements GraphComputer {
    ...
    public OrchidGraphComputer(Graph graph, Supplier<Graph> freshGraph) {
    ...
    @Override public synchronized Future<ComputerResult> submit() {
        ...
        ExecutorService service = Executors.newSingleThreadExecutor(r -> {
            Thread thread = new Thread(r, "orchiddb-computer");
    ...
    private ComputerResult execute(Thread submittingThread,boolean callerTransactionOpen) throws Exception {
        try (AutoCloseable lease = graph instanceof OrchidGraph ? ((OrchidGraph) graph).executionLease(submittingThread,callerTransactionOpen) : () -> {}) {
            interrupted();
    ...
    private ComputerResult executeLeased() throws Exception {
        ...
        if (program != null) {
            ...
            OrchidWorkerMemory workerMemory = new OrchidWorkerMemory(memory);
            while (true) {
                interrupted();
                ...
                while (vertices.hasNext()) {
                    ...
                    worker.execute(ComputerGraph.vertexProgram(vertex, worker),
                            new OrchidMessenger(vertex, messages, worker.getMessageCombiner()), workerMemory);
                }
        ...
        for (MapReduce reducer : reducers) {
            ...
            OrchidMapEmitter emitter = new OrchidMapEmitter(reducer.doStage(MapReduce.Stage.REDUCE));
            ...
            if (reducer.doStage(MapReduce.Stage.REDUCE)) {
                OrchidReduceEmitter reduced = new OrchidReduceEmitter();
