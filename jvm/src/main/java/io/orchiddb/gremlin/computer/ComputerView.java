...
public final class ComputerView implements Graph {
    ...
    public Graph finish(GraphComputer.ResultGraph result, GraphComputer.Persist persist, Supplier<Graph> freshGraph) {
        ...
        try {
            ...
            if (target instanceof OrchidGraph) ((OrchidGraph) target).atomicMutation(copyGraph);
            else copyGraph.run();
            interrupted();
    ...
    private void publishOriginal() {
        if (source instanceof OrchidGraph) {
            try {
                ((OrchidGraph) source).atomicMutation(this::writeOriginalProperties);
