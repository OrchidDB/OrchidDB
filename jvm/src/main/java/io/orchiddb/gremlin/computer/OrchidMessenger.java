 ...
 */
public final class OrchidMessenger<M> implements Messenger<M> {

    private final Vertex vertex;
    private final OrchidMessageBoard<M> messageBoard;
    private final MessageCombiner<M> combiner;

    public OrchidMessenger(final Vertex vertex, final OrchidMessageBoard<M> messageBoard, final Optional<MessageCombiner<M>> combiner) {
        this.vertex = vertex;
...
//        for (final MessageScope messageScope : this.messageBoard.previousMessageScopes) {
            if (messageScope instanceof MessageScope.Local) {
                ...
                final Traversal.Admin<Vertex, Edge> incidentTraversal = OrchidMessenger.setVertexStart(localMessageScope.getIncidentTraversal().get().asAdmin(), this.vertex);
                final Direction direction = OrchidMessenger.getDirection(incidentTraversal);
                ...
                multiIterator.addIterator(StreamSupport.stream(Spliterators.spliteratorUnknownSize(VertexProgramHelper.reverse(incidentTraversal.asAdmin()), Spliterator.IMMUTABLE | Spliterator.SIZED), false)
                        .map((Edge e) -> {
                            ...
                            if (direction.equals(Direction.IN) || direction.equals(Direction.OUT)) {
                                ...
                                vv = e.outVertex().equals(this.vertex) ? e.inVertex() : e.outVertex();
                            }
