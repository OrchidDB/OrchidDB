...
final class OrchidSession implements AutoCloseable {
    ...
    private final OrchidSession owner;
    private final long graphId;
    OrchidSession(String executable, java.nio.file.Path path) {
        ...
        try {
            ...
            },"orchiddb-native-replies");
            reader.setDaemon(true); reader.start();
        } catch(IOException e) { throw new IllegalStateException("Cannot start OrchidDB native store: "+executable,e); }
    }
    /** Borrow a native graph owned by the relational executor over a callback channel. */
    OrchidSession(BufferedReader source, OutputStream sink) {
    ...
    private synchronized Object callForGraph(long graphId,Map<String,Object> request) {
        ...
        try {
            ...
            Object reply=borrowedInput!=null?borrowedInput.readLine():replies.poll(Long.getLong("orchiddb.native.timeoutSeconds",120),TimeUnit.SECONDS);
    ...
    private void drainInterruptedReply() throws IOException {
        long timeout=Long.getLong("orchiddb.native.cancelDrainMillis",5000);
