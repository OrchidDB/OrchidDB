...
public final class OrchidJvm {
    private OrchidJvm() { }
    public static void main(String[] args) throws Exception {
        String executable = System.getenv().getOrDefault("ORCHIDDB_JVM_STORE", "orchiddb-jvm-store");
        Path path = null;
        for (int i=0; i<args.length; i++) {
            ...
            else throw new IllegalArgumentException("Usage: OrchidJvm [--store EXECUTABLE] [--path SNAPSHOT_FILE]");
        }
        ObjectMapper json = new ObjectMapper();
        var graphson = GraphSONMapper.build().version(GraphSONVersion.V3_0).create().createMapper();
        OrchidGraph graph = path==null ? OrchidGraph.open(executable) : OrchidGraph.open(executable,path);
        try (OrchidJvmExecutor executor = new OrchidJvmExecutor(graph);
             BufferedReader input = new BufferedReader(new InputStreamReader(System.in,StandardCharsets.UTF_8))) {
            String line;
            while ((line=input.readLine())!=null) {
                Map<String,Object> response = new LinkedHashMap<>();
                response.put("protocol",1);
                response.put("execution_profile","orchiddb-jvm");
                try {
                    ...
                    if (request.has("bindings")) {
                        ...
                        request.get("bindings").fields().forEachRemaining(entry -> bindings.put(entry.getKey(),
                                OrchidCodec.decode(json.convertValue(entry.getValue(),Object.class),graph)));
                    }
