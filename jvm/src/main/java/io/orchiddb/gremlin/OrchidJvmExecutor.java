...
/**
 * Sequential JVM traversal session over a native OrchidDB store. Scripts are trusted application
 ...
 */
public final class OrchidJvmExecutor implements AutoCloseable {
    private final OrchidGraph graph;
    private final GremlinGroovyScriptEngine engine;
    private final ExecutorService worker = Executors.newSingleThreadExecutor(r -> {
        Thread t = new Thread(r, "orchiddb-jvm-session"); t.setDaemon(true); return t;
    });
    private final ScheduledExecutorService deadlines = Executors.newSingleThreadScheduledExecutor(r -> {
        Thread t = new Thread(r, "orchiddb-jvm-deadline"); t.setDaemon(true); return t;
    });
    private final AtomicBoolean closed = new AtomicBoolean();
    private final Set<CompletableFuture<?>> pending = ConcurrentHashMap.newKeySet();

    public OrchidJvmExecutor(OrchidGraph graph) {
    ...
    public RemoteConnection remoteConnection() {
        return new RemoteConnection() {
            ...
            @Override public void close() { OrchidJvmExecutor.this.close(); }
        };
    }
