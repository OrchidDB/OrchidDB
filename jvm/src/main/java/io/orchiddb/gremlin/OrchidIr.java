...
public final class OrchidIr {
    private OrchidIr() { }
    private static Object encode(Object value,OrchidGraph graph) {
        checkOwnership(value,graph);
        return OrchidCodec.encode(value,graph);
    }
    private static void checkOwnership(Object value,OrchidGraph graph) {
        ...
        try {
...

            {
                ...
                if("Computer".equals(mode)) {
                    response.put("rows",OrchidComputerKernel.execute(request,graph,engine));
                } else if("Sort".equals(mode)) {
                    List<Map<String,Object>> decoded=new ArrayList<>();
                    for(Map<String,Object> row:rows) {
                        Map<String,Object> value=new LinkedHashMap<>();
                        row.forEach((k,v)->value.put(k,OrchidCodec.decode(v,graph)));decoded.add(value);
                    }
                ...
                for(Map<String,Object> row:rows) {
                    ...
                    for(var entry:row.entrySet()) bindings.put(entry.getKey(),OrchidCodec.decode(entry.getValue(),graph));
                    bindings.put("graph",graph); bindings.put("g",graph.traversal());
                    if(request.get("traversers") instanceof List<?> frames)
                        bindings.put("__orchiddb_traverser",OrchidTraverser.create((Map<String,Object>)frames.get(rowIndex),graph));
                    rowIndex++;
