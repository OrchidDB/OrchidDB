...
import static io.orchiddb.gremlin.OrchidCodec.fields;

/**
 * TinkerPop graph backed by a private OrchidDB native process. A graph owns a single
 ...
 */
public final class OrchidGraph implements Graph {
    public static final String DEFAULT_CARDINALITY = "orchiddb.vertex.defaultCardinality";
    private final VertexProperty.Cardinality defaultCardinality;
    public static final String VERTEX_ID_MANAGER = "orchiddb.vertex.idManager";
    public static final String EDGE_ID_MANAGER = "orchiddb.edge.idManager";
    public static final String VERTEX_PROPERTY_ID_MANAGER = "orchiddb.vertexProperty.idManager";
    private final IdManager vertexIdManager,edgeIdManager,vertexPropertyIdManager;
    private final String executable;
    private final OrchidSession session;
    ...
    private final org.apache.tinkerpop.gremlin.structure.service.ServiceRegistry services=OrchidServices.create(this);
    private int bulkLoadDepth;
    private int atomicMutationDepth;
    private long nextNumericId;
    private boolean numericIdsExhausted;
    private final int adjacencyCacheSize=Math.max(0,Integer.getInteger("orchiddb.native.adjacencyCacheSize",4096));
    ...
    private final int propertyCacheSize=Math.max(0,Integer.getInteger("orchiddb.native.propertyCacheSize",16384));
    ...
    private OrchidGraph(String executable,Path path) { this(executable,path,null,VertexProperty.Cardinality.single); }
    private OrchidGraph(String executable,Path path,RuntimeValues family,VertexProperty.Cardinality cardinality) {
    ...
    static OrchidGraph forIr(java.io.BufferedReader source, java.io.OutputStream sink) {
        return new OrchidGraph("IR-owned",null,null,VertexProperty.Cardinality.single,
                IdManager.ANY,IdManager.ANY,IdManager.ANY,new OrchidSession(source,sink));
    }
    private OrchidGraph(String executable,Path path,RuntimeValues family,VertexProperty.Cardinality cardinality,
                      IdManager vertexIds,IdManager edgeIds,IdManager propertyIds,OrchidSession borrowed) {
        ...
        session=borrowed==null?new OrchidSession(executable,path):borrowed;
        ...
        configuration.setProperty(Graph.GRAPH,OrchidGraph.class.getName());
        ...
        configuration.setProperty("orchiddb.native.executable",executable);
        if(path!=null) configuration.setProperty("orchiddb.native.path",path.toString());
    }
    public static OrchidGraph open() {
        String executable=System.getenv("ORCHIDDB_JVM_STORE");
        if(executable==null || executable.isEmpty()) throw new IllegalStateException("Set ORCHIDDB_JVM_STORE to the orchiddb-jvm-store executable");
        return open(executable);
    }
    public static OrchidGraph open(String executable) { return new OrchidGraph(executable,null); }
    public static OrchidGraph open(String executable,Path path) { return new OrchidGraph(executable,path); }
    public static OrchidGraph open(Configuration config) {
        String executable=config.getString("orchiddb.native.executable",System.getenv("ORCHIDDB_JVM_STORE"));
        String path=config.getString("orchiddb.native.path",null);
        OrchidGraph graph=new OrchidGraph(executable,path==null?null:Path.of(path),null,
    ...
    public OrchidGraph freshGraph() {
        if(closed || runtime.closed) throw new IllegalStateException("Graph family is closed");
        return new OrchidGraph(executable,null,runtime,defaultCardinality,vertexIdManager,edgeIdManager,vertexPropertyIdManager,session.isBorrowed()?session.fork():null);
    }
    public Object encodeValue(Object value) { return OrchidCodec.encode(value,this); }
    public Object decodeValue(Object value) { return OrchidCodec.decode(value,this); }
    ...
    private static final class RuntimeValues {
        ...
        final Set<OrchidGraph> members=java.util.concurrent.ConcurrentHashMap.newKeySet();
    }
    ...
    private Object call(Map<String,Object> request) {
        ...
        try {
            ...
            if(committedRead) {
                if(!committedReadsSupported) throw new UnsupportedOperationException("Native store lacks committed snapshot reads; use a compatible orchiddb-jvm-store binary");
    ...
    private Object nextNumericId(IdManager manager) {
        while(true) {
            ...
            Object encoded=OrchidCodec.encode(id);
    ...
    <E> E decode(Object value) { return (E)OrchidCodec.decode(value,this); }
    ...
    Object encodeId(Object id,String type) {
        ...
        return OrchidCodec.encode(manager.convert(id));
    }
    ...
    @Override public GraphComputer compute() {
        try {
            return (GraphComputer)Class.forName("io.orchiddb.gremlin.computer.OrchidGraphComputer")
                .getConstructor(Graph.class,Supplier.class).newInstance(this,(Supplier<Graph>)this::freshGraph);
        } catch(ReflectiveOperationException e) { throw new UnsupportedOperationException("OrchidGraphComputer module is unavailable",e); }
    }
    ...
    public void abortFamily() {
        List<OrchidGraph> members;
        synchronized(runtime) { runtime.closed=true; members=new ArrayList<>(runtime.members); }
        for(OrchidGraph member:members) member.abort();
    }
    public void closeFamily() {
        List<OrchidGraph> members;
        synchronized(runtime) { runtime.closed=true; members=new ArrayList<>(runtime.members); }
        RuntimeException failure=null;
        for(OrchidGraph member:members) try { member.close(); } catch(RuntimeException e) { if(failure==null) failure=e; else failure.addSuppressed(e); }
    ...
    private final class NativeTransaction extends AbstractThreadLocalTransaction {
        ...
        NativeTransaction() { super(OrchidGraph.this); }
    ...
    public static final class NativeGraphFeatures implements Features.GraphFeatures {
        ...
        @Override public boolean supportsComputer() { try { Class.forName("io.orchiddb.gremlin.computer.OrchidGraphComputer"); return true; } catch(ClassNotFoundException e) { return false; } }
