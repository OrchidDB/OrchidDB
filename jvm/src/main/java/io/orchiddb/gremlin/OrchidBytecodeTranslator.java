...
public final class OrchidBytecodeTranslator extends GroovyTranslator.LanguageTypeTranslator {
  public final Map<String,Object> bindings=new LinkedHashMap<>();
  private final OrchidCallbackRegistry callbacks;
  public OrchidBytecodeTranslator(OrchidCallbackRegistry callbacks){super(false);this.callbacks=callbacks;}
  @Override protected org.apache.tinkerpop.gremlin.process.traversal.Script convertToScript(Object value) {
   if(!(value instanceof Enum<?>)&&(value instanceof java.util.function.Supplier<?>||value instanceof java.util.function.Function<?,?>||value instanceof java.util.function.BiFunction<?,?,?>)) {
    String name="__orchiddb_java_binding_"+bindings.size();
  ...
  @Override protected org.apache.tinkerpop.gremlin.process.traversal.Script produceScript(VertexProperty<?> value) {
   String name="__orchiddb_java_binding_"+bindings.size();
   bindings.put(name,Map.of("type","vertex_property","id",OrchidCodec.encode(value.id()),"owner",Map.of("type","vertex","id",OrchidCodec.encode(value.element().id()),"label",value.element().label()),"key",value.key()));return script.append(name);
  }
