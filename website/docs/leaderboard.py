...
def render(cases, get_result, runs, root, download):
    ...
    for suite, title, products in SUITES:
        ...
        for index, product in enumerate(order):
            ...
            html.append(f'<td>{rank_label}</td><th scope="row">{PRODUCTS[product]}</th><td><strong>{passed:,}</strong> / {len(subset):,}</td><td>{change}</td><td>{gap_label}</td><td><a href="/downloads/conformance/{product}-{suite}.json">{escape(timestamp)}</a></td></tr>')
        html.append('</tbody>')
