...
def render(cases, get_result, runs, root, download):
    ...
    html = ['<section id="leaderboard" class="leaderboard"><h2>Current leaderboard</h2>',
            '<p>Ranked by passed upstream scenarios within each language. OrchidDB is compared as one product, with one recorded outcome per scenario from a single suite run; execution details are available in the evidence. Peer-only passes are cases another compared engine passes that this engine has not passed.</p>',
    ...
    for suite, title, products in SUITES:
        ...
        html.append('</tbody>')
    html.append('</table></div><p>OrchidDB passed runtime sums recorded scenario time for passed cases only, including setup and assertions. Failed and other non-passing cases are excluded.</p><p class="leaderboard-downloads"><a href="/downloads/conformance/leaderboard.json">Leaderboard and gap cases JSON</a> · <a href="/downloads/conformance/parity-baseline.json">Starting baseline</a></p></section>')
