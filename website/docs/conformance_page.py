...
def render(out):
 ...
 html.append('<p class="matrix-legend"><span><i class="complete"></i>All passed</span><span><i class="mixed"></i>Some passed</span><span><i class="failed"></i>No passes; failures recorded</span><span><i class="unknown"></i>Not evaluated</span></p><p class="matrix-note">Cells show passed / total upstream scenarios. Crabgraph is shown as one product, with one recorded outcome per scenario from a single suite run. Select a cell for individual results and timings. All features are listed below.</p><p id="comparison-count" role="status"></p><div class="feature-browser" id="cases"><div class="feature-stage"><p id="empty-stage" hidden>No features match these filters.</p>')
