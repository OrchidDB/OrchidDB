...
def render_java_evidence(download):
 ...
 html=['<section class="java-evidence" id="java-provider"><h2>Java provider tests</h2><p>These separately recorded Java provider tests are supplemental evidence. Their outcomes are not added to the matrix or product leaderboard. The comparison uses the single OrchidDB suite run linked above.</p>']
...
<p><strong>RDF semantics.</strong> Both engines use the original W3C fixtures and expected artifacts. Query and update syntax are checked separately from execution. SELECT comparisons preserve datatypes, unbound values and global blank-node identity, normalize numeric values using Oxigraph 0.5.11, and honor the manifest’s ordering and REDUCED cardinality rules. Graph results use isomorphism; updates compare complete datasets and named graph identity. Raw expected and actual terms are included in the downloads. Protocol, entailment and federation cases retain their recorded statuses.</p>
<p><strong>Fixtures and interfaces.</strong> SQLg uses PostgreSQL and upstream TinkerFactory fixtures. PuppyGraph maps disposable external PostgreSQL fixture tables; mutation outcomes refer to that configuration. Neo4j Community executes the original Cypher statements in Cypher 5 mode. It also materializes PuppyGraph's GIVEN fixtures in separate local runs; expected answers always come from the TCK. Apache Jena executes SPARQL against TDB2 using the same W3C fixture and result files as OrchidDB. Jena is licensed under Apache 2.0; Neo4j Community uses GPLv3. Fixtures that cannot be represented faithfully are excluded with a reason. OrchidDB uses a local repository build in hybrid mode and its RDF dataset API. Native result metadata preserves graph identities, numeric widths, paths, sets and typed map keys. Structured Cypher errors are compared against the required type, detail and execution phase.</p>
