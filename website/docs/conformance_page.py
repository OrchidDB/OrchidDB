...
<p><strong>Gremlin execution profiles.</strong> Native Rust results exercise the Crabgraph traversal engine. JVM OLTP runs pinned TinkerPop traversal and callback machinery over the native CrabGraph provider. GraphComputer runs vertex programs over that provider. Each profile retains its own exclusions, source and binary hashes, and raw results. The product leaderboard counts each Gremlin scenario once when any Crabgraph execution profile passes. For 15 upstream Gherkin placeholders, the adapter executes the corresponding original Apache JUnit test. Each result identifies the Java source, assertion outcome, timing and build; these scenarios are counted once through the JVM profile. Other Java provider tests and supplemental tests remain separate and are not added to scenario totals.</p>
