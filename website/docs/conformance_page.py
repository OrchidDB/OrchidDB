...
<p><strong>Gremlin execution profiles.</strong> Native Rust results exercise the Crabgraph traversal engine. JVM OLTP runs pinned TinkerPop traversal and callback machinery over the native CrabGraph provider. GraphComputer runs vertex programs over that provider. Each profile retains its own exclusions, source and binary hashes, and raw results. The product leaderboard counts each Gremlin scenario once when any Crabgraph execution profile passes. Original Java provider tests and supplemental tests appear in the separate Java evidence table; their counts are not added to Gherkin totals.</p>
