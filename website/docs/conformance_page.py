...
def render(out):
 ...
 chunks.append('<h2 id="tested-queries">Tested queries</h2><p>'+str(len(probes))+' probes cover patterns, paths, joins, subqueries, collections, aggregates, nulls, functions, mutations and traversal state. A pass applies to the displayed query and fixture. Query rejection can reflect a dialect difference; it does not establish that every equivalent expression is unavailable.</p><p class="timing-note">Timing: median client wall time over three consecutive runs. This is a four-node fixture, with different embedded/network overheads. Environments appear in the column headings; times from different hosts are not directly comparable.</p><p class="comparison-key"><span class="status pass">Matches expected</span> · <span class="status mismatch">Result differs</span> different rows · <span class="status query-error">Query rejected</span> engine error · No adapter: this language was not run for that product.</p>')
 def table_start(caption):return '<div class="comparison-scroll" tabindex="0" role="region" aria-label="'+caption+'"><table class="comparison-table"><caption>'+caption+'</caption><thead><tr><th scope="col">Feature / exact probe</th>'+''.join('<th scope="col">'+v+'<small class="engine-environment">'+e(results[p]['build']['version']+' · '+results[p].get('execution_environment',{}).get('system','')+' '+results[p].get('execution_environment',{}).get('architecture',''))+'</small></th>' for p,v in PRODUCTS.items())+'</tr></thead><tbody>'
...
<p>Conformance runs locally against the free engines. Results are committed as versioned JSON, and GitHub Actions builds and publishes this static page from those committed results. Each cell is tied to a probe hash; changed expectations cannot silently reuse old results. The version details identify the exact tested build and environment.</p>
