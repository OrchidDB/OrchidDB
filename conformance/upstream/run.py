...
JVM_ENGINES=('orchiddb-jvm','orchiddb-computer')
...
def jvm_build(classpath):
 ...
 if len(artifacts)!=1:raise RuntimeError('Expected one pinned orchiddb-jvm jar in conformance classpath')
...
def orchiddb_binary():
 return Path(os.environ.get('CONFORMANCE_ORCHIDDB_BINARY',str(REPO/'target/debug/upstream'))).resolve()
class Process:
...
class Gremlin:
 ...
 def run(self,case):
  ...
  if self.engine=='orchiddb' and self.process is not None and self.process.p.poll() is not None:raise RuntimeError('The single OrchidDB adapter exited; no restart is permitted')
...
def main():
 ...
 if args.engine=='orchiddb' and args.suite=='tinkerpop':
  def classpath_identity(classpath):
   artifacts=[]
   for entry in classpath.split(os.pathsep):
    path=Path(entry)
    if path.is_dir():
     files=sorted(path.rglob('*.class'))
     artifacts.append({'path':str(path.resolve()),'classes':len(files),'sha256':hashlib.sha256('\n'.join(str(f.relative_to(path))+':'+hashlib.sha256(f.read_bytes()).hexdigest() for f in files).encode()).hexdigest()})
    else:artifacts.append(file_identity(path))
   return artifacts
 ...
 if args.suite=='tinkerpop':
  ...
  if args.engine=='orchiddb-jvm':content['execution_profile']['placeholder_assertions']='Pinned original Apache JUnit counterparts; per-case source and execution evidence recorded'
